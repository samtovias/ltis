...
function kthFold(path1,path2,folders,i,k)
    path_out = fullfile(path2,folders{i},'selected','linkage_trees');
	fprintf('%s\n',path_out);
    ...
    w = 0.5; 
    tstart = tic;
    [subset,solution,curves] = ltis(X,Y,w,dataset_name,exp); 
    telapsed = toc(tstart);
    X = subset.bXS;
    Y = subset.bYS; 
    P = subset.bPS; 
    x = solution.bx;
    f = curves.curve_bf(end);
    iter = curves.iter; 
