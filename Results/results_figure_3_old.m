...
ha = tight_subplot(7,6,[.005 .005],[.01 .1],[.1 .01]);
for i = 1:N
    ids = id(:,i);
...
f = gcf;
% Guardar figura como PDF, SVG o PNG 
% set(f,'Units','Inches'); 
% set(f,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[f.Position(3) f.Position(4)]);
% name_file = 'syndata_figure';
% print(f, name_file, '-depsc');
% Change eps to pdf: 
% https://www.zamzar.com/converters/document/eps-to-pdf/
% PDF
%print('-painters',name_file,'-dpdf','-r0','-bestfit');
% SVG
%print('-painters',name_file,'-dsvg','-r0');
% PNG
%saveas(f,strcat(name_file,'.png'));
